...
st.write(
    "Sistema de geração automática de horários utilizando "
    "Algoritmo Genético."
)
...
if "melhor_grade" in st.session_state:
    ...
    grafico = pd.DataFrame({
        "Geração": range(1, len(historico) + 1),
        "Conflitos": historico
    })
    ...
    if total_conflitos == 0:
        ...
        st.success(
            "✅ Solução sem conflitos encontrada!"
        )

    else:

        st.warning(
            f"⚠️ A solução possui {total_conflitos} conflito(s)."
        )